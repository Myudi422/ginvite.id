...
export default function InvitationView({ data }: { data: any }) {
  ...
  const endDate = new Date(eventDate.getTime() + 3600000)
    .toISOString()
    .replace(/-|:|\.\d+/g, "");
  const calendarUrl = `https://www.google.com/calendar/render?action=TEMPLATE&dates=${startDate}/${endDate}&text=${encodeURIComponent(
    opening.title
        ...
        {!isOpen && (
          ...
          >
            {[decorations.topLeft,
              ...
              decorations.bottomRight].map((src, i) => (
              <motion.img
                key={i}
                src={src}
                alt=""
                className={`absolute ${
                  ["top-0 left-0","top-0 right-0","bottom-0 left-0","bottom-0 right-0"][i]
                } w-48 h-48`}
                animate={{
                  x: i % 2 ? [30, 0, 30] : [-30, 0, -30],
                  y: i < 2 ? [-30, 0, -30] : [30, 0, 30],
                }}
