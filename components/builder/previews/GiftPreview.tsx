...
export default function GiftPreview({ props, style }: P) {
  ...
  const renderTitle = () => (
    <div className={`text-center space-y-2 ${getAnimClass(1)}`}>
      <h2 className="text-2xl font-semibold flex items-center justify-center gap-2"
        style={{ color: textColor, fontFamily: fontHead }}>
        <FaGift style={{ color: accent }} />
        {sectionTitle}
      </h2>
      <p className="text-sm opacity-80 max-w-sm mx-auto" style={{ color: textColor }}>
  ...
  const renderGiftCategoryTabs = () => {
    ...
    return (
      <div className={`flex justify-center gap-2 mb-6 ${getAnimClass(2)}`}>
        <button
          type="button"
          onClick={() => setActiveGiftTab('bank')}
          className="px-4 py-2 text-xs font-bold rounded-full transition-all border shadow-sm focus:outline-none"
          style={{
  ...
  const renderBankDetails = () => {
    ...
    return (
      <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2 justify-center mb-1">
            {banks.map((acc, idx) => (
              <button
                key={acc.id || idx}
                type="button"
                className="px-3 py-1 text-xs rounded-lg border font-semibold transition-all focus:outline-none shadow-sm"
                style={{
        ...
        <div
          className="p-5 rounded-2xl border backdrop-blur-sm relative overflow-hidden transition-all shadow-md"
          style={{
        ...
        >
          <div className="flex items-start justify-between">
            <div className="space-y-2">
              ...
              <div className="pt-1">
                <p className="text-lg font-bold tracking-wider leading-none" style={{ color: textColor }}>
                  {currentAccount.account_number || 'Nomor Rekening'}
                </p>
                <p className="text-xs opacity-75 mt-1" style={{ color: textColor }}>
            ...
            <button
              ...
              className="p-2.5 rounded-xl transition-all hover:scale-105 shadow-sm border focus:outline-none"
              style={{
  ...
  const renderAddressDetails = () => {
    ...
    return (
      <div className="space-y-3">
        <div
          className="p-5 rounded-2xl border backdrop-blur-sm relative overflow-hidden transition-all shadow-md text-left"
          style={{
            backgroundColor: bgColor + '90',
            borderColor: accent + '40',
          }}
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="space-y-2 flex-1">
              ...
              <div className="pt-1 space-y-1">
                <p className="text-sm font-semibold opacity-90" style={{ color: textColor }}>
                  Penerima: <strong className="font-bold">{addressRecipient || '-'}</strong>
                </p>
                {addressPhone && (
                ...
                <p className="text-xs opacity-90 leading-relaxed border-t border-gray-150/40 pt-2 mt-2 break-words" style={{ color: textColor }}>
                  {addressText}
                </p>
              </div>
            </div>
            ...
            <button
              ...
              className="p-2.5 rounded-xl transition-all hover:scale-105 shadow-sm border focus:outline-none shrink-0"
              style={{
                backgroundColor: accent,
                color: bgColor,
                borderColor: accent,
  ...
  const renderConfirmationForm = () => {
    ...
    return (
      <form onSubmit={handleSubmit} className="space-y-4 pt-2">
        <div className="text-xs uppercase tracking-widest font-extrabold mb-1 opacity-80 text-center" style={{ color: accent }}>
          Formulir Konfirmasi Transfer
        </div>

        {error && (
          <div className="p-3 rounded-xl bg-red-100/90 flex items-start gap-2 text-red-800 text-xs text-left">
            <span>⚠️ {error}</span>
          </div>
        )}
        {success && (
          <div className="p-3 rounded-xl bg-green-100/90 flex items-start gap-2 text-green-800 text-xs text-left">
        ...
        <div className="space-y-3">
          ...
          <div className="relative">
            <FaUser className="absolute top-3.5 left-3 text-xs opacity-60" style={{ color: accent }} />
            <input
              id="bank-nama"
              type="text"
              disabled={loading}
          ...
          {/* Nominal Transfer */}
          <div className="relative">
            <FaMoneyBillWave className="absolute top-3.5 left-3 text-xs opacity-60" style={{ color: accent }} />
            <input
              id="bank-jumlah"
              type="text"
              disabled={loading}
  ...
  const renderFloating = () => (
    <div className="space-y-6 w-full max-w-lg mx-auto">
      ...
      {activeGiftTab === 'bank' && hasBanks && (
        <div className="space-y-6">
          <div className={`w-full bg-white/95 dark:bg-slate-900/95 backdrop-blur-md rounded-3xl shadow-2xl p-6 border border-white/40 transform hover:-translate-y-1 transition-transform duration-300 ${getAnimClass(2)}`}>
            {renderBankDetails()}
          </div>
