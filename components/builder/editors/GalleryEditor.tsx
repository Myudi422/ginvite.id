...
import { Field, FieldGroup, Input, ColorInput, Select, Toggle, AddButton } from '../ui/EditorFields';
...
export default function GalleryEditor({ props, onChange }: P) {
  ...
  const removeImage = async (i: number) => {
    const url = images[i];
    const newUploadedList = (props.new_uploaded_images as string[]) || [];
    const isNew = newUploadedList.includes(url);
    ...
    const updatedUploaded = newUploadedList.filter(u => u !== url);

    onChange({
      ...props,
      images: newImages,
      new_uploaded_images: updatedUploaded
    });
  };
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="space-y-2">
        {renderHeader('gallery', 'Galeri Foto', Type)}
        {openGroups.gallery && (
          <div className="p-3 bg-white border border-gray-100 rounded-2xl space-y-4 shadow-sm">
            <Field label="Template Layout Galeri">
              <Select
                value={layoutTemplate}
                onChange={v => set('layout_template', v)}
                options={[
                  { value: 'grid', label: 'Grid Kolom Klasik' },
                  { value: 'masonry', label: 'Masonry Grid Unik' },
                  { value: 'slider', label: 'Carousel Slider Horizontal' },
                  { value: 'grid_mosaic', label: 'Artistic Mosaic Collage' },
                ]}
              />
            </Field>

            {(layoutTemplate === 'grid' || layoutTemplate === 'masonry') && (
            ...
            <div className="space-y-4">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">
                Daftar Foto ({images.length} / 6)
              </p>
              
              <div className="space-y-3">
                {images.map((img, i) => (
                  <div key={i} className="flex gap-2 items-center bg-gray-50/50 p-2.5 rounded-2xl border border-gray-100">
                    <div className="flex-1 min-w-0">
                      <ImagePicker
                        ...
                        onChange={(v, isNew) => {
                          ...
                          if (isNew) {
                            const newUploaded = [...((props.new_uploaded_images as string[]) || [])];
                            newUploaded.push(v);
                            onChange({
                              ...props,
                              images: newImages,
                              new_uploaded_images: newUploaded
                            });
                          } else {
                            setImage(i, v);
                          }
                ...
                {images.length < 6 ? (
                  <AddButton label="Tambah Foto Baru" onClick={addImage} />
                ) : (
                  <p className="text-[10px] text-gray-400 font-medium italic text-center py-2 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                    Batas maksimal foto tercapai (Maksimal 6 foto).
                  </p>
                )}
              </div>
            </div>
