...
import { Field, Input, Toggle, Select, ColorInput } from '../ui/EditorFields';
import ImagePicker from '../ui/ImagePicker';
import { deleteImageFromBackblaze } from '@/app/actions/backblaze';
import { ChevronDown, CheckSquare, Image as ImageIcon, Trash2 } from 'lucide-react';
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="space-y-2">
        {renderHeader('rsvp', 'Pengaturan RSVP', CheckSquare)}
        {openGroups.rsvp && (
          <div className="p-3 bg-white border border-gray-100 rounded-2xl space-y-4 shadow-sm">
            ...
            <div className="flex items-center justify-between py-1">
              <div>
                <p className="text-xs font-semibold text-gray-700">Aktifkan RSVP</p>
                <p className="text-[10px] text-gray-400">Tampilkan form ucapan & konfirmasi</p>
              </div>
              <Toggle
                checked={(props.enabled as boolean) ?? true}
            ...
            <Field label="Batas Konfirmasi (opsional)">
              <Input
                type="date"
                value={(props.deadline as string) || ''}
            ...
            <Field label="Template Layout">
              <Select
                ...
                options={[
                  { value: 'classic', label: 'Klasik (Form + Komentar)' },
                  { value: 'card', label: 'Card Terpusat' },
                  { value: 'minimal', label: 'Minimalis Bersih' },
                  { value: 'floating', label: 'Floating Form' },
                ]}
              />
            </Field>
            ...
            {/* Jumlah komentar ditampilkan awal */}
            <Field label={`Komentar Awal Ditampilkan: ${(props.initial_comments as number) ?? 5}`}>
              <input
                type="range" min={3} max={20} step={1}
                value={(props.initial_comments as number) ?? 5}
                onChange={e => set('initial_comments', parseInt(e.target.value))}
