...
export default function ThemeContainer({ children, maxWidth = '420px', className = '' }: ThemeContainerProps) {
  return (
    <div
      className={`mx-auto relative min-h-screen shadow-2xl overflow-hidden bg-[var(--t6-bg-main)] ${className}`}
      style={{
        maxWidth,
        color: 'var(--t6-text-secondary, #555555)'
      }}
    >
      {/* 
...
export function ThemeHeader({ children, size = 'xl', className = '', align = 'center', style }: ThemeHeaderProps) {
  ...
  return (
    <h2
      ...
      style={{
        color: 'var(--t6-text-primary, #4A719C)',
        fontFamily: 'var(--t6-font-heading)',
        ...style
      }}
    >
      {children}
...
export function ThemeText({ children, variant = 'body', color = 'white', align = 'center', className = '' }: ThemeTextProps) {
  ...
  const colorThemeMap = {
    white: 'var(--t6-text-white, #FFFFFF)',
    gray: 'var(--t6-text-secondary, #555555)',
    gold: 'var(--t6-text-accent, #F9EFCB)',
    black: 'var(--t6-text-primary, #4A719C)'
  };
...
export function ThemeButton({ children, onClick, variant = 'primary', className = '', disabled }: ThemeButtonProps) {
  ...
  const getVariantStyles = () => {
    switch (variant) {
      case 'primary':
        return {
          background: 'var(--t6-grad-button, #4A719C)',
          color: 'var(--t6-text-white, #ffffff)',
          border: 'none',
          boxShadow: '0 4px 15px rgba(74, 113, 156, 0.3)'
        };
      case 'outline':
        return {
          border: '1px solid var(--t6-border-glass, #4A719C)',
          color: 'var(--t6-text-primary, #4A719C)',
