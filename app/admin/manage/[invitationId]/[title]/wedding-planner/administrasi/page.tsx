            ...
            <div className="sticky top-0 z-10 bg-white/80 backdrop-blur-md border-b border-rose-100 shadow-sm">
                <div className="flex items-center justify-between p-4 max-w-5xl mx-auto">
                    <div className="flex items-center gap-2">
                        <button onClick={() => router.back()} className="p-2 rounded-xl hover:bg-rose-50"><ChevronLeft className="h-5 w-5 text-rose-600" /></button>
                        <div>
                            <h1 className="text-base font-bold text-gray-800">📋 Administrasi</h1>
                            <p className="text-xs text-rose-500 truncate">{decodeURIComponent(title)}</p>
                        </div>
                    </div>
            ...
            <div className="max-w-5xl mx-auto p-4 pb-10 space-y-4">
                ...
                {tasks.length > 0 && (
                    <div className="bg-white rounded-2xl p-5 shadow-sm border border-rose-100">
                        <div className="flex justify-between mb-2">
                            ...
                            <p className="text-sm font-bold text-rose-600">{doneTasks}/{tasks.length}</p>
                        </div>
                ...
                {showForm && (
                    <div className="bg-white rounded-2xl p-5 shadow-sm border border-rose-100 space-y-3">
                        ...
                        <input value={newTask} onChange={e => setNewTask(e.target.value)} placeholder="Nama tugas (mis: Urus buku nikah)" className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-rose-300" />
                        <div>
                            <label className="text-xs text-gray-500 mb-1 block">Tenggat Waktu (opsional)</label>
                            <input type="date" value={dueDate} onChange={e => setDueDate(e.target.value)} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-rose-300" />
                        </div>
                        <div className="flex gap-2">
                            <button onClick={() => handleAdd(newTask)} disabled={saving} className="flex-1 bg-rose-500 hover:bg-rose-600 text-white text-sm font-medium py-2.5 rounded-xl transition-colors disabled:opacity-50">{saving ? 'Menyimpan...' : '✓ Simpan'}</button>
                            ...
                            <div className="grid grid-cols-1 gap-2">
                                {remaining.map(dt => (
                                    <button key={dt} onClick={() => handleAdd(dt)} disabled={saving}
                                        className="text-left bg-white rounded-xl px-4 py-3 border border-rose-100 shadow-sm text-sm text-gray-700 hover:bg-rose-50 hover:border-rose-300 transition-all flex items-center justify-between group disabled:opacity-50">
                                        <span>{dt}</span>
                                        <Plus className="h-4 w-4 text-rose-400 opacity-0 group-hover:opacity-100 transition-opacity" />
                                    </button>
                                ))}
                            </div>
                        </div>
                    ...
                    <div className="space-y-2">
                        {tasks.map(task => (
                            <div key={task.id} className={`bg-white rounded-xl px-4 py-3 border shadow-sm flex items-center gap-3 transition-all ${task.is_done ? 'border-emerald-100 opacity-75' : 'border-gray-100'}`}>
                                <button onClick={() => handleToggle(task.id, task.is_done)} className={`w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors ${task.is_done ? 'bg-emerald-500 border-emerald-500' : 'border-gray-300 hover:border-rose-400'}`}>
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    {task.due_date && <p className="text-xs text-rose-600 mt-0.5">📅 {task.due_date}</p>}
                                </div>
