...
export default function ListUser() {
  ...
  const generateWhatsAppUrl = (user: User) => {
    const url = user.preview_url || `https://papunda.com/undang/${user.user_id}/${user.title}`;
            ...
            {users.slice(0, visibleUsers).map((user) => (
              <TableRow key={user.id} className="hover:bg-pink-50/50 transition-colors">
                <TableCell className="font-medium text-gray-700">
                  <div>{user.id}</div>
                  {user.source === 'builder' ? (
                    <span className="inline-block mt-1.5 px-1.5 py-0.5 rounded bg-purple-100 text-purple-800 text-[9px] font-bold uppercase tracking-wider">
                      ✨ Builder
                    </span>
                  ) : (
                    <span className="inline-block mt-1.5 px-1.5 py-0.5 rounded bg-pink-100 text-pink-800 text-[9px] font-bold uppercase tracking-wider">
                      Legacy
                    </span>
                  )}
                </TableCell>
                        ...
                        <div className="space-y-4">
                          <div className="space-y-2">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-gray-500 capitalize">{user.source || 'legacy'}</p>
                            </div>
                            <div>
